...
{
    //histan_NSLog(@"%@", NSStringFromSelector(_cmd));
    [super drawRect:rect];
...
- (void)setText:(NSString *)text andCenter:(CGPoint)center
{
    ...
    CGSize fontSize =[self.text sizeWithFont:self.font
                                    forWidth:200
                               lineBreakMode:NSLineBreakByTruncatingTail];
    //histan_NSLog(@"%f   %f", fontSize.width, fontSize.height);
...
- (void)setup
{
    ...
    _actionView = [[UIControl alloc] initWithFrame:self.frame];
    ...
    [_actionView addTarget:self action:@selector(removeHighlightedColor) forControlEvents:UIControlEventTouchCancel |
...
{
    //histan_NSLog(@"添加事件（UnderLineLabel中）");
